...
#	$Form.Close()
#	& $path"\propwin.ps1"
foreach($app in $xml.Monitor.ChildNodes )
		{ 
			if($app.SelectSingleNode("monitorID").InnerText -ne "0")
			{
				$result = [System.Windows.Forms.MessageBox]::Show("You already have " +$app.Name+ " monitor." +[char]13+"Do you want to add new?","",[System.Windows.Forms.MessageBoxButtons]::YesNo)
				
				if($result -eq [System.Windows.Forms.DialogResult]::Yes)
				{
					$xml.Monitor.SelectSingleNode($app.name).SelectSingleNode("monitorID").InnerText = "0"
...
$MainPanel.Size = New-Object System.Drawing.Size(267, 334);
