...
if oFso.FileExists(cfgfile) then
  oConf.Load(cfgfile)
	If oConf.parseError.errorCode <> 0 Then    
		wscript.Echo oConf.parseError.errorCode    
		wscript.Echo oConf.parseError.reason    
		wscript.Echo oConf.parseError.line
	End If  	
else
...
select case cmd
  case "get"
    Set oNode = oConf.documentElement.selectSingleNode("SecretKey")
