function Add-MonitisResellerCustomMonitor 
{
    <#
    .Synopsis
    ...
    # Specifies id of the order in scope of which monitor will be added. This is required parameter for reseller's client accounts.  
    # If any command connects to Monitis, the ApiKey and SecretKey will be cached    
    [Parameter(ValueFromPipelineByPropertyName=$true)]
    [string]$orderId
    )
    ...
    process {
        ...
        $tag = $name
        $postData = "apiKey=$script:ApiKey&authToken=$script:AuthToken&validation=token&timestamp=$timestamp&version=2&output=json&name=$name&action=addMonitor&tag=$tag&orderId=$orderId&resultParams=$resultParams&monitorParams=$monitorParams"
