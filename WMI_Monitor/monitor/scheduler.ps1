...
#Parse system time to scheduler standart
...
function rdBtn_Minute_CheckedChanged
{
	...
	$txtBx_recur.Value = 1
	$txtBx_repeat.Value = 1
	$lbl_min.Location = New-Object System.Drawing.Point(($txtBx_recur.Location.X+50),$txtBx_recur.location.y)
 	...
 	else
 	{
		if($rdBtn_Daily.checked -eq $true)
		{
			...
			else
			{
				...
				elseif(($d.Contains("windows xp")) -or ($d.Contains("server 2003")))
				{
					...
					{
						$Command = "schtasks /create /tn $taskname /tr ""C:\WINDOWS\system32\wscript.exe $taskrun $argumentfile"" /sc $schedul  /mo $modifier /sd $startdate /st $starttime /ed $enddate /ru $username /rp $password >status"
						cmd.exe /c $Command
						$Form.Close()
						$Form.Visible = $false
						$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
		...
		if($rdBtn_Weekly.Checked -eq $true)
		{
			...
			{	
				...
				if (($d.Contains("windows 7")) -or ($d.Contains("server 2008")))
				{
					$Command = "schtasks /create /tn $taskname /tr ""$taskrun $argumentfile"" /sc $schedul  /mo $modifier /d $day /RI $repeat /du $duration /sd $startdate /st $starttime /ed $enddate >status"
					cmd.exe /c $Command
					$Form.Close()
					$Form.Visible = $false
					$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
				...
				elseif(($d.Contains("windows xp")) -or ($d.Contains("server 2003")))
				{
					if(($password -ne "") -and ($username -ne ""))
					{
						...
						$Command = "schtasks /create /tn $taskname /tr ""C:\WINDOWS\system32\wscript.exe $taskrun $argumentfile"" /sc $schedul  /mo $modifier /d $day /sd $startdate /st $starttime /ed $enddate /ru $username /rp $password >status"
						cmd.exe /c $Command
						$Form.Close()
						$Form.Visible = $false
						$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
		...
		{			
			...
			else
			{
				...
				if (($d.Contains("windows 7")) -or ($d.Contains("server 2008")))
				{
					$Command = "schtasks /create /tn $taskname /tr ""$taskrun $argumentfile"" /sc $schedul /RI $repeat /sd $startdate /st $starttime /du $duration >status"			
					cmd.exe /c $Command
					$Form.Close()
					$Form.Visible = $false
					$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
		...
		if($rdBtn_Monthly.Checked -eq $true)
		{
			...
			else
			{
				...
				if (($d.Contains("windows 7")) -or ($d.Contains("server 2008")))
				{
					$Command = "schtasks.exe /create /tn $TaskName /tr ""$taskrun $argumentfile"" /sc $Schedule /mo $Modifier /d $day /m $Months /st $StartTime  /ri $Interval /du $duration >status"
					cmd.exe /c $Command
					$Form.Close()
					$Form.Visible = $false
					$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
				...
				elseif(($d.Contains("windows xp")) -or ($d.Contains("server 2003")))
				{
					if(($password -ne "") -and ($username -ne ""))
					{
						$Command = "schtasks.exe /create /tn $TaskName /tr ""C:\WINDOWS\system32\wscript.exe $taskrun $argumentfile"" /sc $Schedule /mo $Modifier /d $day /m $Months /st $StartTime /ru $username /rp $password >status"
						cmd.exe /c $Command
						$Form.Close()
						$Form.Visible = $false
						$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
		...
		if ($rdBtn_onStart.Checked -eq $true)
		{
			...
			if (($d.Contains("windows 7")) -or ($d.Contains("server 2008")))
			{
				$myCmd = "schtasks.exe /Create /TN $TaskName /SC $Schedule /tr ""$taskrun $argumentfile"" >status"
				cmd.exe /c $myCmd 
				$Form.Close()
				$Form.Visible = $false
				$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
			...
			elseif(($d.Contains("windows xp")) -or ($d.Contains("server 2003")))
			{
				if(($password -ne "") -and ($username -ne ""))
				{
					$myCmd = "schtasks.exe /Create /TN $TaskName /SC $Schedule /tr ""C:\WINDOWS\system32\wscript.exe $taskrun $argumentfile"" /ru $username /rp $password >status"
					cmd.exe /c $myCmd 
					$Form.Close()
					$Form.Visible = $false
					$xml1.Monitor.ChildNodes.Item(0).SelectSingleNode("task").InnerText = $taskname
